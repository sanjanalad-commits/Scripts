...
pre <- read.csv('/Users/sanjanalad/Downloads/New H2 2025 Behind The Scenes Survey Data  - Pre-Webinar.csv', stringsAsFactors = FALSE)
...
# ---------------------------- DEEP DIVE VERSION ----------------------------
attend <- read.csv('/Users/sanjanalad/Downloads/Final H2 2025 Behind The Scenes Survey Data  - Post-Web Attendee.csv', stringsAsFactors = FALSE)
noattend <- read.csv('/Users/sanjanalad/Downloads/Final H2 2025 Behind The Scenes Survey Data  - Post-Web Non-Attendee.csv', stringsAsFactors = FALSE)
pre <- read.csv('/Users/sanjanalad/Downloads/Final H2 2025 Behind The Scenes Survey Data  - Pre-Webinar.csv', stringsAsFactors = FALSE)
...
# --- Fix "No" logic ---
if (ncol(pre) >= 12) {
  pre[[11]] <- ifelse(pre[[12]] != "" | pre[[13]] != "" | pre[[14]] != "", "No", pre[[11]])
}

# --- Clear column 4 if condition not met ---
if (ncol(pre) >= 5 && ncol(pre) >= 4) {
  pre[, 5][pre[, 4] != "I’m familiar with Uber Crew and have participated in an event before"] <- ""
}
...
rbind$QUESTION <- gsub("Do you plan on attending Behind The Scenes with Crew on October 16th ",
                       "Do you plan on attending Behind The Scenes with Crew on October 16th?", rbind$QUESTION)
...
rbind$QUESTION <- gsub("Why didn’t you attend Behind The Scenes with Crew ",
                       "Why didn’t you attend Behind The Scenes with Crew?", rbind$QUESTION)
rbind$QUESTION <- gsub("Did you receive a notification to register for Behind The Scenes with Crew ",
                       "Did you receive a notification to register for Behind The Scenes with Crew?", rbind$QUESTION)
